...
function start-up(){
    ...
    git clone https://github.com/rzuckerm/phix-docker-images.git docker-image

    cp -R $PWD/bin $PWD/docker-image

    cp $PWD/Dockerfile $PWD/docker-image/Dockerfile

    cp $PWD/install_phix.sh $PWD/docker-image/install_phix.sh

    cd $PWD/docker-image

    sudo docker build --build-arg="PHIX_VERSION=$(cat PHIX_VERSION)" -t $docker_img_name .
